...
\begin{wrapfigure}[13]{L}{0.51\textwidth}
    \centering
    \vspace{-10pt}
...
\qquad Cleveland se mimo jiné podílel na tvorbě řády technik pro prohlížení komplexních datových sad s více proměnnými, kterým se říká \textit{Trellis Graphics} nebo \textit{Trellis Plots}. Technika obdržela svůj název \textit{trellis} kvůli obvyklému výsledku řady obdélníkových grafů, připomínajících zahradní mříž. Na obrázku \ref{fig08} je ukazka *trellis* grafu, zobrazující údaje o emisích motoru. [@trellisplot]
...
\qquad \textit{The Grammar of Graphics} říká, že statistické grafy jsou mapováním z dat k estetickým atributům (barva, tvar, velikost) nebo geometrickým objektům (body, linie, sloupce). Graf také může obsahovat statistickou transformaci dat a být vykreslen ve specifickém souřadnicovém systému. *Faceting* může být použito k vygenerování stejného grafů pro různé podmnožiny datasetu. Kombinace těchto nezávislých komponent tvoří grafiku. Jednotlivé komponenty tvořící graf, dle Wilkinsonovy syntaxe, lze zapsat následovně:
